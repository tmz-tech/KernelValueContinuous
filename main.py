...
class CalculateValues:
    ...
    def fit_state(self, order: int = 3, cv_x=True, search_interval_x=np.linspace(0.05, 0.06, 100), cv_k_x: int = 20):
        ...
        # Compute the basis functions using the optimal bandwidth
        ...
        # Estimate the reward using the computed basis functions and state data
        self.r_pi_est = self.estimation_reward(self.h_x, self.scaled_state_df, self.reward_df)
    ...
    def fit_action(self, cv_a=True, search_interval_a=np.linspace(0.05, 0.06, 100), cv_k_a: int = 20):
        ...
        # Estimate the reward and compute the basis functions for the next state-action pairs
        self.est_r_sa_actual, self.est_next_psi_SA_actual = self.estimation_r_SA_next_psi_SA(self.h_x, self.h_a, self.scaled_state_df, self.action_df, self.scaled_next_x_df, self.reward_df, self.c_basis)
    ...
    def estimation_r_SA_next_psi_SA(self, h_x, h_a, scaled_state_df, action_df, scaled_next_x_df, reward_df, basis):
        """
