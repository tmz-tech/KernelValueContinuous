...
class BasisNextExpect(BaseEstimator):
    ...
    def __init__(self, bandwidth=1.0, alpha=0.1):
        ...
        self.alpha = alpha # Set the regularization parameter for numerical stability
...
class BasisNextExpect2(BaseEstimator):
    ...
    def basis_next_expect(self, x, h_x, basis):
        ...
        BNE_num = np.sum(basis_next_df.values * np.prod(K/h_x, axis=1)[:, np.newaxis], axis=0)
        # Compute the denominator: the sum of the kernel values
        BNE_denom = np.sum(np.prod(K/h_x, axis=1))
...
class BasisNextSAExpect(BaseEstimator):
    ...
    alpha : float, default=0.1
        ...
        could otherwise result in unstable or biased estimates.
    """
    def __init__(self, bandwidth_x=1.0, bandwidth_a=1.0, alpha=0.1):
        """
