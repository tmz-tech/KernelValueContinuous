...
class est_policy(BaseEstimator):
    ...
    def pi_est(self, x, a, h_x, h_a, epsilon=1e-10):
        ...
        density_num = np.mean(K_x*K_a) # Weighted sum of kernels (joint state-action density)
        density_denom = np.mean(K_x) # Marginal density of states
...
class est_r_pi(BaseEstimator):
    ...
    bandwidth : float, default=1.0
        The bandwidth parameter for the Gaussian kernel, which controls the 
    ...
    def nw_est(self, x, h_x, epsilon=1e-10):
        ...
        nw_num = np.mean(self.R_ * K_x[:, np.newaxis]) # Element-wise multiplication between response values and kernel weights
        # Compute the denominator: the sum of kernel weights (i.e., marginal density)
        nw_denom = np.mean(K_x) 
...
class est_r_pi_w(BaseEstimator):
    ...
    This class extends the basic Nadaraya-Watson kernel regression by introducing a
    window parameter that determines the lag between input data and response values.
    It adjusts the input and response data accordingly to fit with this window.
    ...
    def nw_est(self, x, h_x, epsilon=1e-10):
        ...
        # Apply the Gaussian kernel to the pairwise differences
        K_x = np.exp(-0.5 * np.sum(u_x**2, axis=1)) / ((2 * np.pi)**(self.X_.shape[1] / 2) * h_x**self.X_.shape[1])
        ...
        nw_num = np.mean(self.R_w * K_x_w[:, np.newaxis]) # Element-wise multiplication of response values with kernel weights
        
        # Compute the denominator: the sum of kernel weights (i.e., marginal density based on the full data)
        nw_denom = np.mean(K_x) # Kernel weight sum for the marginal density
...
class est_r_sa(BaseEstimator):
    ...
    ----------
    X_ : ndarray of shape (n_samples, n_features)
        The input state data used for fitting.
    A_ : ndarray of shape (n_samples,)
        The action labels corresponding to each sample in X_.
    R_ : ndarray of shape (n_samples,)
        The response values associated with the state-action pairs.
    """
    def __init__(self, bandwidth_x=0.1, bandwidth_a = 0.1):
        self.bandwidth_x = bandwidth_x # Bandwidth for state features
        self.bandwidth_a = bandwidth_a # Bandwidth for action features
    ...
    def nw_est(self, x, a, h_x, h_a, epsilon = 1e-10):
        ...
        nw_num = np.mean(self.R_ * K[:, np.newaxis]) # Weighted sum of responses
        nw_denom = np.mean(K) # Sum of kernel weights (marginal density)
...
class est_r_pi_sa_w(BaseEstimator):
    ...
    def __init__(self, bandwidth_x=0.1, bandwidth_a=0.1):
        # Initialize the estimator with the specified bandwidth for state and action
        self.bandwidth_x = bandwidth_x # Bandwidth for state features
        self.bandwidth_a = bandwidth_a # Bandwidth for action features
    ...
    def nw_est(self, x, a, h_x, h_a, epsilon = 1e-10):
        ...
        # Compute the numerator: the weighted sum of truncated response values
        nw_num = np.mean(self.R_w * (K_x_w*K_a_w)[:, np.newaxis]) # Element-wise multiplication of response values with kernel weights
        
        # Compute the denominator: the sum of kernel weights (i.e., marginal density based on the full data)
        nw_denom = np.mean(K_x*K_a) # Kernel weight sum for the marginal density
